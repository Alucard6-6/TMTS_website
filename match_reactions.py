...
def refiningarrheniusBS(TMTSmodel,Rsmiles):
    ...
    for model in TMTSmodel:
        ...
        rdmolfiles.MolToPDBFile(m3,"/mount/src/tmts_website2/sym.pdb",flavor=16)
        callplatonsym()
        extsymRmodel=tradsym(foundsym("/mount/src/tmts_website2/sym.lis"))
        ...
        file = open("/mount/src/tmts_website2/sym.pdb","r")
        textinput = file.read()
        textinputcorrige="COMPND    \n"
        textfinal = textinputcorrige+textinput
        textfinal=textfinal.replace(" H "," F ")
        textfinal=textfinal.replace(" H"," F")
        file = open("/mount/src/tmts_website2/sym.pdb","w")
        file.write(textfinal)
        callplatonsym()
        extsymR=tradsym(foundsym("/mount/src/tmts_website2/sym.lis"))
...
def refiningarrhenius(TMTSmodel,Rsmiles,Psmiles,path,cycle):
    ...
    for model in TMTSmodel:
        ...
        for isomer in isomerssmi:
            ...
            file.write(textfinal)
            callplatonsym()
            extsymiso=tradsym(foundsym("/mount/src/tmts_website2/sym.lis"))
...
def symfromsmi(smiles):
    ...
    # file2.close()
    file2 = open("/mount/src/tmts_website2/sym.pdb","r")
    textinput = file2.read()
    textinputcorrige="COMPND    \n"
    textfinal = textinputcorrige+textinput
    textfinal=textfinal.replace(" H "," F ")
    textfinal=textfinal.replace(" H"," F")
    file2 = open("/mount/src/tmts_website2/sym.pdb","w")
    file2.write(textfinal)
    callplatonsym()
    symiso=tradsym(foundsym("/mount/src/tmts_website2/sym.lis"))
...
def addnewkinetic(dicofreactions):
    ...
    AllChem.EmbedMolecule(m3R) 
    AllChem.MMFFOptimizeMolecule(m3R)
    rdmolfiles.MolToPDBFile(m3R,"/mount/src/tmts_website2/sym.pdb",flavor=16)
    ...
    file.close()
    #callplatonsym()
    extsymtest=tradsym(foundsym("/mount/src/tmts_website2/sym.lis"))
