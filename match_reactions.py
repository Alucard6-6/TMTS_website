...
def getextsym(TMTSmodel, Rsmiles):
    session_id = str(uuid.uuid4())[:8]
    pdb_path = f"./sym_{session_id}.pdb"
    ...
    for model in TMTSmodel:
        ...
        rdmolfiles.MolToPDBFile(m3R, pdb_path, flavor=16)
        with open(pdb_path, "r") as f:
            textinput = f.read()
        
        textinputcorrige = f"COMPND    ./base.xyz\nAUTHOR    GENERATED BY OPEN BABEL 3.1.0\n{textinput}"
...
def matchreaction_BS(Rsmiles,Psmiles):
    ...
    if match==1:
        ...
        Arrhenius=None
    fc, lcmodel, extsym = getextsym(TMTSmodel, Rsmiles)
    return Arrhenius, fc, lcmodel, extsym 
    ...
    for model in TMTSmodel:
        ...
        for isomer in isomerssmi:
            ...
            with open(pdb_path, "w") as f:
                f.write(textinputcorrige)
            callplatonsym(pdb_path)
            extsymiso=tradsym(foundsym(lis_path))
            if os.path.exists(pdb_path): os.remove(pdb_path)
            if os.path.exists(lis_path): os.remove(lis_path)
...
def symfromsmi(smiles):
    ...
    AllChem.MMFFOptimizeMolecule(m3)
    rdmolfiles.MolToPDBFile(m3, pdb_path, flavor=16)
    with open(pdb_path, "r", encoding="utf-8") as file:
        textinput = file.read()
        
    textinputcorrige = f"COMPND    ./base.xyz\nAUTHOR    GENERATED BY OPEN BABEL 3.1.0\n{textinput}"
    textinputcorrige = textinputcorrige.replace(" H ", " F ")

    with open(pdb_path, "w", encoding="utf-8") as file2:
        file2.write(textinputcorrige)

    callplatonsym(pdb_path)
    symiso = tradsym(foundsym(lis_path))
...
def addnewkinetic(dicofreactions):
    ...
    for reaction in dicofreactions:
        ...
        if typeofreaction=="isoCH":
            ...
            arrhenius, fc, LCmodel, extsymR=matchreaction_BS(dicofreactions[reaction][0][0],dicofreactions[reaction][1])
            if arrhenius!=None:
                nBS+=1
    ...
    # print("nhabs",nhabs)
    return updateddic, fc, LCmodel, extsymR
