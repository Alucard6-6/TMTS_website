...
def refiningarrheniusBS(TMTSmodel,Rsmiles):
    ...
    for model in TMTSmodel:
        ...
        ids = rdDistGeom.EmbedMultipleConfs(m3, numConfs=30, randomSeed=42)
        results = AllChem.MMFFOptimizeMoleculeConfs(m3, maxIters=1000)
        ...
        rdmolfiles.MolToPDBFile(
            m3,"sym.pdb", confId=best_conf_id, flavor=16
        )
        callplatonsym()
...
def addnewkinetic(dicofreactions):
    ...
    # file = open("sym.pdb","r")
    # textinput = file.read()
    # textinputcorrige="COMPND    C:/Users/liu2/Downloads/platon_console(1)/base.xyz\nAUTHOR    GENERATED BY OPEN BABEL 3.1.0\n"
    # textfinal = textinputcorrige+textinput
    # file = open("sym.pdb","w")
    # file.write(textfinal)
    # file.close()
