...
*                        Version 03-02-26                                *
*                                                                        *
* (C) 1992-2026 - A.L. Spek, Utrecht University, The Netherlands         *
*                                                                        *
*                          ====================                          *
*                                                                        *
...
int xwin_(int *x, int *y, int * z, int *ind, char *buf) {
  ...
  static int           n_sopt[MOUSE_OPTIONS  + 1]; /* number of sub-boxes */
  static int           m_sopt[MOUSE_OPTIONS  + 1]; /* active sub-box */ 
  static int           on_sopt[MOUSE_OPTIONS + 1]; /* old number os sub-boxes */
  static int           om_sopt[MOUSE_OPTIONS + 1]; /* old active sub-box */ 
  static int           cm_opt[MOUSE_OPTIONS  + 1];
  unsigned int         wx = 0, wy = 0;              /* window location     */
  unsigned int         border_width = 0;            /* no border 	   */
  unsigned long        foreground_pixel;
  unsigned long        theWindowMask;
  static float         menu_step, menu_y, xstep, ystep, yystep,ibx, iby;
  int                  menu_x;
  int                  iex;
  char                 window_name[25];
...
/* show menu options */
        ...
        XDrawLine (theDisplay, theWindow, theGC, (int)ibx, 0, (int)ibx,
                   dheight);
...
/* loop over menu options */
	for (i = 0; i < MOUSE_OPTIONS + 1; i++) {
          ...
          XSetForeground (theDisplay, theGC, colors[ibaw]);
/*
* Previous Sub-menu Section Bars (Delete)
*/
...
/*
* Previously Active Horizontal Sub-menu Bars (Delete) 
*/
          j = on_sopt[i];
          if (j > 0) {
            for (k = 1; k < km + 1; k++) {
              yystep = (float)k * ystep / (float)km;
              XDrawLine(theDisplay, theWindow, theGC,
                        (int)(ibx + (j - 1) * xstep),
                        (int)(iby - yystep),
                        (int)(ibx + j * xstep),
          ...
          XSetForeground(theDisplay, theGC, colors[1 -ibaw]);
/*
* Current Sub-menu Section Bars (Show)
*/
          if (m_sopt[i] > 0) {
...
/*
* Currently Active Horizontal Sub-menu Bars (Show) 
*/
          j = n_sopt[i];
          on_sopt[i] = j;
          if (j > 0) {
...
*/
  ...
  */
      ...
      n_sopt[*ind - 100] = *z / 100;  /* load number of sub_boxes */
      m_sopt[*ind - 100] = *z % 100;  /* load active sub-box number */
      cm_opt[*ind - 100] = *y;        /* load box colour */
      return(0);
  /*
  * IND > 199
  */
